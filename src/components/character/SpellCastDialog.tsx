...
import { Sparkles, Zap, Clock, Target, Component, ChevronUp } from "lucide-react";
...
interface SpellCastDialogProps {
  ...
  onCast: (spellLevel: number, castAtLevel: number) => void;
...
export function SpellCastDialog({
  ...
  const handleCast = () => {
    ...
    onCast(spell.level, selectedLevel);
    
    const isUpcast = selectedLevel > spell.level;
    toast.success(`${spell.name} lançado!`, {
      description: isUpcast 

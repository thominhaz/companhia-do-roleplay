...
const THEME_STYLES = {
  default: { border: "border-primary/30", bg: "bg-primary/5", iconBg: "bg-primary/20", iconText: "text-primary", textMuted: "text-muted-foreground", buttonBg: "bg-primary/10 text-primary" },
};
...
function MasterCampaignCard({ 
  ...
  return (
    <div 
      className={cn("bg-surface-1/80 backdrop-blur-md border border-border/30 rounded-2xl p-5 relative overflow-hidden cursor-pointer shadow-depth-sm hover:shadow-depth-md hover:scale-[1.01] transition-all")}
      onClick={onClick}
    >
      <div className="absolute top-0 right-0 w-40 h-40 bg-primary opacity-[0.03] rounded-full -mr-16 -mt-16" />
      <div className="absolute bottom-0 left-0 w-32 h-32 bg-primary opacity-[0.03] rounded-full -ml-12 -mb-12" />
      
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={cn("w-14 h-14 rounded-xl border flex items-center justify-center text-2xl", theme.border, theme.iconBg)}>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="bg-muted/50 rounded-lg p-2.5">
            <p className="text-xs text-muted-foreground mb-0.5">Jogadores</p>
        ...
        <div className="flex gap-2 mb-4">
          <button 
            className="flex-1 py-2 rounded-lg text-xs font-medium bg-primary/10 text-primary border border-primary/20"
        ...
        <div className="flex items-center justify-between pt-3 border-t border-border/30">
          ...
          <button className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-primary/60 bg-transparent text-primary hover:bg-primary hover:text-primary-foreground transition-colors">
            Gerenciar
          </button>
        </div>
      </div>
...
function PlayerCampaignCard({ campaign, masterName, onClick, onChatClick, onNotesClick, onCombatClick, onLeaveClick }: { campaign: CampaignDB; masterName?: string; onClick: () => void; onChatClick: () => void; onNotesClick: () => void; onCombatClick: () => void; onLeaveClick: () => void }) {
  return (
    <div 
      className="bg-surface-1/80 backdrop-blur-md rounded-xl p-4 border border-border/30 cursor-pointer hover:border-primary/30 transition-colors shadow-depth-sm"
      onClick={onClick}
    >
      <div className="flex items-center gap-3 mb-3">
        <div className="w-12 h-12 rounded-lg border border-secondary/40 bg-secondary/10 flex items-center justify-center">
      ...
      <div className="flex gap-2 mb-3">
        <button 
          className="flex-1 py-2 rounded-lg text-xs font-medium bg-secondary/10 text-secondary border border-secondary/20"
          onClick={(e) => { e.stopPropagation(); onChatClick(); }}
        >
          <MessageCircle className="w-3 h-3 inline mr-2" />Chat
        </button>
